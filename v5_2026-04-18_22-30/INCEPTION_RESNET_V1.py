...
Why face-pretrained over ImageNet:
  ImageNet → learns general objects, faces are ~5 of 1000 classes
  VGGFace2 → 3.3M images of faces only, forces learning subtle facial structure
  (eye shape, brow position, lip corners) — the exact features emotion lives in

Architecture: InceptionResNetV1 → 512-dim face embeddings → 3-class head
All v4 training techniques retained: MixUp, EMA, label smoothing, cosine LR,
gradient clipping, WeightedRandomSampler, FocalLoss.
"""
...
# ── Training ──────────────────────────────────────────────────────────────────
# Phase 1: head only — backbone frozen, face features used as-is
for p in model.head.parameters():
    p.requires_grad = True
hist1 = train_phase(model, lr=1e-3, epochs=15, patience=5,
                    desc="Phase 1: head only (face backbone frozen)...")

# Phase 2: unfreeze last 30 modules — adapt deeper face features to emotions
unfreeze_last_n(model, 30)
hist2 = train_phase(model, lr=1e-4, epochs=25, patience=8,
                    desc="Phase 2: fine-tuning last 30 layers...")

# Phase 3: unfreeze last 60 modules — full fine-tuning
